...
CFG <- list(
  ...
  na_vars          = c("Glucose", "BloodPressure", "SkinThickness",
                       "Insulin", "BMI"),
  outcome_var      = "Outcome",
  miss_rates       = c(0.10, 0.20, 0.30),
  mechanisms       = c("MCAR", "MAR", "MNAR"),
  n_seeds          = 100,        # set to 10 for a quick smoke test
  max_iter_chain   = 10,
  conv_tol         = 1e-3,
  rf_ntree         = 100,
  ...
  clinical_bounds  = list(
    Glucose       = c(50, 500),
    BloodPressure = c(40, 200),
    SkinThickness = c(3,  80),
    Insulin       = c(0,  846),
    BMI           = c(10, 80)
  )
)
...
# -----------------------------------------------------------------------------
load_pima <- function(path = CFG$data_path) {
  df <- readr::read_csv(path, show_col_types = FALSE)
  for (v in CFG$na_vars) df[[v]][df[[v]] == 0] <- NA
  # Use only complete cases as the "ground truth" pool for simulation.
  # NOTE: this is a known limitation discussed in the paper - the original
  #       zeros are likely MNAR, so the complete-case pool is not the full
  #       population. We address it by ALSO running MNAR simulations.
...
simulate_missing <- function(data, vars, rate, mechanism, seed) {
  ...
  for (v in vars) {
    ...
    if (mechanism == "MCAR") {
      ...
      lp <- scale(data$Age)[, 1] + scale(data$Pregnancies)[, 1]
      p  <- plogis(lp - mean(lp))
...
plausibility_joint <- function(imputed, truth, mask,
  ...
  hull_idx <- tryCatch(geometry::convhulln(obs_pts),
                       error = function(e) NULL)
  if (is.null(hull_idx))
    return(data.frame(JointOutsidePct = NA_real_))
  inside <- geometry::inhulln(geometry::convhulln(obs_pts, options = "Tv"),
                              imp_pts)
  data.frame(JointOutsidePct = mean(!inside) * 100)
}
...
# -----------------------------------------------------------------------------
main <- function() {
  complete_data <- load_pima()
  res <- run_experiment(complete_data)

  saveRDS(res, "imputation_results_raw.rds")
  ...
  cat("\nDone. Raw results saved to imputation_results_raw.rds\n")
  invisible(res)
}
